...
public final class LdshadowladyHeadTemplate extends StructureComponentTemplate {
	private static final ResourceLocation ID = new ResourceLocation(MonstersAndPets.MODID, "ldshadowlady_head");
	...
	public LdshadowladyHeadTemplate() {}

	public LdshadowladyHeadTemplate(TemplateManager mgr, BlockPos pos, Rotation rotation) {
		super(0);
	...
	protected void handleDataMarker(String function, BlockPos pos, World world, Random rng, StructureBoundingBox sbb) {
		switch (function) {
			...
			case "ldshadowlady_chest": {
				world.setBlockState(pos,Blocks.AIR.getDefaultState(),2);
				BlockPos chestPos = pos.down();
				if (sbb.isVecInside(chestPos)) {
					TileEntity entity = world.getTileEntity(chestPos);
					if (entity instanceof TileEntityChest) {
						((TileEntityChest) entity).setLootTable(MonstersAndPetsLootTableList.LDSHADOWLADY_CHEST, rng.nextLong());
					}
				}
				break;
			}
