...
public final class WitchHouseGenerator extends MapGenStructure {
	private static final String NAME = "WitchHouse";

	private static final String COMPONENT_NAME = "WtchHs";

	private static final int SEED = 768362;
	...
	public WitchHouseGenerator(ChunkGeneratorOverworld generator) {
	...
	public static void init() {
		MapGenStructureIO.registerStructure(WitchHouseGenerator.Start.class, NAME);
		MapGenStructureIO.registerStructureComponent(WitchHouseTemplate.class, COMPONENT_NAME);
	}
	...
	public static final class Start extends StructureStart {
		...
		public static Start create(World world, ChunkGeneratorOverworld generator, Random rng, int chunkX, int chunkZ) {
			...
			int y = primer.findGroundBlockIdx(8,8);
			BlockPos pos = new ChunkPos(chunkX, chunkZ).getBlock(12, y-1, 12);
			MinecraftServer server = world.getMinecraftServer();
			TemplateManager mgr = world.getSaveHandler().getStructureTemplateManager();
			return new Start(chunkX, chunkZ, Collections.singletonList(new WitchHouseTemplate(mgr, pos, rotation)));
		}
	}
}